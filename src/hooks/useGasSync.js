...
export function useGasSync() {
  ...
  const timers = useRef({ notes: null, tasks: null, journal: null, daily: null });
  ...
  useEffect(() => {
    ...
    async function loadAll() {
      ...
      if (result && result.success) {
        ...
        console.warn('GAS fetch failed, falling back to localStorage');
  ...
  const updateNotes = useCallback((newNotes) => {
    const updated = typeof newNotes === 'function' ? newNotes(notes) : newNotes;
    setNotes(updated);
  ...
  const updateJournal = useCallback((newJournal) => {
    const updated = typeof newJournal === 'function' ? newJournal(journal) : newJournal;
    setJournal(updated);
    if (initialized.current) {
  ...
  const updateDaily = useCallback((newDaily) => {
    const updated = typeof newDaily === 'function' ? newDaily(daily) : newDaily;
    setDaily(updated);
    if (initialized.current) {
