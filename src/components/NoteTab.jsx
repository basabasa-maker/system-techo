...
export default function NoteTab({ notes, onUpdate }) {
  ...
  // Filter by search, then sort: pinned first, then by date
  const filteredNotes = allNotes
    ...
    .sort((a, b) => {
      const aPinned =
        a.pinned === true || a.pinned === "TRUE" || a.pinned === "true";
      const bPinned =
        b.pinned === true || b.pinned === "TRUE" || b.pinned === "true";
  ...
  const isPinned = (note) =>
    note.pinned === true || note.pinned === "TRUE" || note.pinned === "true";

  const handleTogglePin = (note) => {
    const updated = allNotes.map((n) =>
      n.id === note.id ? { ...n, pinned: isPinned(n) ? "FALSE" : "TRUE" } : n,
    );
    onUpdate(updated);
  };
              ...
              >
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <h4 className="font-bold text-[#1e3a5f] text-sm truncate">
                      {note.title}
                    </h4>
